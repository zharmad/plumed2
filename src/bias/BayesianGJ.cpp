...
double BayesianGJ::getEnergy(double sigma){
  ...
  ene = 0.5*ene/s/s + std::log(sigma) + static_cast<double>(ndata_)*std::log(s*sqrt2pi);
  return temp_ * ene;
}
...
void BayesianGJ::calculate(){
  ...
  for(unsigned i=0;i<getNumberOfArguments();++i){
    ...
    ene += getArgument(i); 
    // set derivatives
    setOutputForce(i, -0.5*temp_/s/s);
  };
  // add normalizations and priors
  ene = 0.5*ene/s/s + std::log(sigma_) + static_cast<double>(ndata_)*std::log(s*sqrt2pi);
  // set value of the bias
  valueBias->set(temp_*ene);
  // set value of data uncertainty
  valueSigma->set(sigma_);
  // and of the harmonic restraint
  valueKappa->set(temp_/s/s);
}
